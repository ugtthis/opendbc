...
# If you need conversions and CarParams (originally used in docs_definitions.py),
# you'll have to import them. Please adapt to your actual paths:
...
class Kit(EnumBase):
    red_panda_kit = BasePart(
        "CAN FD panda kit",
        parts=[Device.red_panda, Accessory.harness_box, Cable.usb_a_2_a_cable, Cable.usbc_otg_cable, Cable.obd_c_cable_1_5ft]
    )
...
class CommonFootnote(Enum):
    EXP_LONG_AVAIL = CarFootnote(
        ...
        Column.LONGITUDINAL, True, False
    )
...
class CarDocs:
    ...
    def init(self, CP: CarParams, all_footnotes=None):
        ...
        if self.car_parts.parts:
            ...
            def display_func(parts):
                return '<br>'.join([
                    f"- {parts.count(part)} {part.value.name}"
                    for part in sorted(set(parts), key=lambda part: str(part.value.name))
                ])

            hardware_col = (
                f"""<details><summary>Parts</summary>
                <sub>{display_func(parts_docs)}<br>{buy_link}</sub>
                </details>"""
            )
            if len(tools_docs):
                hardware_col += (
                    f"""<details><summary>Tools</summary>
                    <sub>{display_func(tools_docs)}</sub>
                    </details>"""
                )
    ...
    def get_detail_sentence(self, CP):
        if not CP.notCar:
            sentence_builder = (
                """openpilot upgrades your <strong>{car_model}</strong> with automated lane centering{alc} and
                adaptive cruise control{acc}."""
            )

            if self.min_steer_speed > self.min_enable_speed:
                if self.min_steer_speed > 0:
                    alc = f" <strong>above {self.min_steer_speed * CV.MS_TO_MPH:.0f} mph</strong>,"
                else:
                    alc = " <strong>at all speeds</strong>,"
            else:
                alc = ""
